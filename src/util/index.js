...
export function createReducer (initialparams, reducerMap) {
  return (state = initialparams, action) => {
    ...
    return reducer ? reducer(state, action.payload ? action.payload : {}, action.params,action) : state
  }
}
//给 reducer 添加模块前缀名
export const createReduceOf = (map,module) =>{ 
  if(isEmpty(map) || !module) return map
  const prefix = module.toUpperCase();
  return Object.keys(map).reduce((out,key)=>{
    out[[prefix,key].join('_')] = map[key]
    return out
  },{})
}
//给 action 添加模块前缀名
export function createActionOf(module){
  module = module?module.toUpperCase():"";
  return function(action){
    return {...action,type:[module,action.type].join('_')}
  }
}
...
export const ctx = process.env.__CONTEXT__ ||  __CONTEXT__ || '';