    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                this.props.changeUserConfig(data)
            }
        });
    }
    getFormItem(){
                ...
                <div>
                    ...
                    >
                        {getFieldDecorator('DEPARTMENT_NAME', {
                            rules: [{ required: true, message: 'Please input your note!' }],
                            initialValue: configData[4].title
                        })(
                            <Input />
                        )}
                    </FormItem>
                    ...
                    >
                        {getFieldDecorator('ROLE_NAME', {
                            rules: [{ required: true, message: 'Please input your note!' }],
                            initialValue: configData[5].title
                        })(
                            <Input />
                        )}
                    </FormItem>
...
Userconfig = Form.create({})(Userconfig)

function mapStateToProps(state,ownProps){
    const userConfig = state.get('system.manage').get('system.config').get('user.config');
    return {
        isLoading: userConfig.isLoading,
        configdata: userConfig.reps
    }
}
